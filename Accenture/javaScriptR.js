...
function creaSchede(arr) {
    ...
    informazioni.innerHTML = '<div class="indietro"> <a class="prova" href="restaurants.html"> <img src="freccia.png" style="width:42px;height:42px;"></a></div>'
    ...
    for (i = 0; i < arr.length; i++) {
        ...
        box_preview_dettaglioTelefono_text = document.createTextNode("Phone: " + arr[i].phone);
        ...
        var box_preview_dettaglioEmail = document.createElement("div");
        box_preview_dettaglioEmail.className = "dettaglioScheda";
        box_preview_dettaglioEmail_text = document.createTextNode("Country: " + arr[i].address.country);
        box_preview_dettaglioEmail.appendChild(box_preview_dettaglioEmail_text);

        box_preview_tabellaScheda.appendChild(box_preview_dettaglioEmail);
...
function visualizza(idScheda, listaSchede) {
    for (i = 0; i < listaSchede.length; i++) {
        if (idScheda == listaSchede[i].id) {
            ...
            street2 = document.createTextNode("Street Address: " + listaSchede[i].address.streetAddress);
            document.querySelector(".tabellaDettaglio").appendChild(street2);

            space = document.createElement("t");
            document.querySelector(".tabellaDettaglio").appendChild(space);

            email = document.createTextNode("Email: " + listaSchede[i].email);
            document.querySelector(".tabellaDettaglio").appendChild(email);
